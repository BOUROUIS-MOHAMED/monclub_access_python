        ...
        {notificationVisible ? (
          <div className="event-notification-wrap">
            <div className="event-notification-box">
              ...
              <h2 className="notification-title">{notificationTitle || "Gym notification"}</h2>
              <p className="notification-text">
                {notificationDescription || "Stay hydrated, control your breathing, and prepare for the next class."}
              </p>

              <div className="chips">
                <span className="chip-primary">Visible for 5s</span>
                <span className="chip-secondary">Live from gym</span>
              </div>
            </div>
          </div>
        ) : (
...
function VideoCard({
  ...
  showAdInside?: boolean;
  compact?: boolean;
  visualSrc?: string | null;
  visualMimeType?: string | null;
}) {
  const isVideo = (visualMimeType ?? "").toUpperCase().startsWith("VIDEO") ||
  ...
  return (
    <div className={`video-card card ${compact ? "compact" : ""}`}>
      <div className="video-panel">
        <div className="video-content" style={{ position: "relative", overflow: "hidden" }}>
          {/* Real scheduled content — always rendered (muted; audio handled separately) */}
          {visualSrc && !showAdInside && (
            isVideo ? (
              <video
                key={visualSrc}
                src={visualSrc}
          ...
          <div style={{ position: "relative", zIndex: 1, display: "flex", flexDirection: "column", height: "100%" }}>
            <div className="video-top">
              ...
              <div className="placeholder-badge">{showAdInside ? "Ad mode" : visualSrc ? "Live" : "Placeholder"}</div>
            </div>
...
function MediaCard({
  ...
  const title = audioTitle || "MonClub TV";
  const progress = Math.max(0, Math.min(100, audioProgressPercent ?? 0));

  return (
    <div className={`media-card card ${textOnly ? "text-only" : ""} ${compact ? "compact" : ""}`}>
      {!textOnly && (
        <div className="media-art">
          <div className="media-tag">● {audioTitle ? "Now Playing" : "Everyday Playlist"}</div>
        </div>
      )}
      ...
      <div className="media-player">
        <div>
          ...
          <h3 className="media-title" style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{title}</h3>
          <p className="media-subtitle">Gym speakers • Cardio zone</p>

          <div className="media-progress-wrap">
            <div className="media-progress-top">
              <span>{audioTitle ? `${Math.round(progress)}%` : "00:00"}</span>
              <span>{audioTitle ? "live" : "00:00"}</span>
            </div>
            <div className="media-progress">
              <div className="media-progress-fill" style={{ width: `${progress}%`, transition: "width 1s linear" }} />
            </div>
          </div>
        </div>
...
export type SmartHomeDashboardPageProps = {
  audioTitle?: string | null;
  audioProgressPercent?: number;
  visualSrc?: string | null;
  visualMimeType?: string | null;
...
export default function TvDashboardPage({
  ...
  const [pendingNotification, setPendingNotification] = useState<{ title: string; description: string } | null>(null);
  ...
  // Poll for screen messages sent from the form
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        if (msg && msg.id !== lastShownMessageIdRef.current) {
          ...
          setPendingNotification({ title: msg.title, description: msg.description ?? "" });
          setNotificationNonce((v) => v + 1);
          // Clear the notification data after display time
          window.setTimeout(() => {
            setPendingNotification(null);
          }, (msg.displayDurationSec ?? 5) * 1000 + 600);
        }
      } catch {
  ...
  const content = useMemo(() => {
    const eventBlock = (
      <EventCarousel
        ...
        notificationTitle={pendingNotification?.title}
        notificationDescription={pendingNotification?.description}
      />
    );
    ...
    const mediaBlock = <MediaCard audioTitle={audioTitle} audioProgressPercent={audioProgressPercent} />;
    const textOnlyMediaBlock = <MediaCard textOnly compact audioTitle={audioTitle} audioProgressPercent={audioProgressPercent} />;
    const normalVideoBlock = <VideoCard showAdInside={false} visualSrc={visualSrc} visualMimeType={visualMimeType} />;
    const adInVideoBlock = <VideoCard showAdInside={true} visualSrc={visualSrc} visualMimeType={visualMimeType} />;
  ...
  }, [activeAdMode, audioProgressPercent, audioTitle, currentQuote, eventIndex, events, notificationVisible, pendingNotification, visualMimeType, visualSrc]);
