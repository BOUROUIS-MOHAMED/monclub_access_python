...
export default function TvUpdatePage() {
  ...
  useEffect(() => {
    fetchStatus();
    getTvVersionInfo().then(setVersionInfo).catch(() => {});
    ...
    <ScrollArea className="h-full">
      <div className="max-w-2xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-xl font-semibold text-foreground">MonClub TV</h1>
          ...
          <div className="rounded-xl border border-primary/20 bg-gradient-to-b from-primary/5 to-transparent overflow-hidden">
            ...
            <div className="p-5 space-y-5">
              ...
              <div className="space-y-2.5">
                {status.releaseDate && <MetaRow icon={Calendar} label="Released" value={formatDate(status.releaseDate)} />}
                {status.availableUntil && <MetaRow icon={CalendarClock} label="Available until" value={formatDate(status.availableUntil)} />}
                {status.sizeBytes && <MetaRow icon={HardDrive} label="Size" value={formatBytes(status.sizeBytes)} />}
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-2">
                  {!status.downloaded && !isDownloading && (
                    <Button className="gap-2 flex-1" onClick={handleDownload} disabled={downloading}>
        ...
        {!updateAvailable && status && (
          <div className="rounded-xl border border-border bg-card/50 p-6 text-center space-y-2">
            ...
            <p className="text-[12px] text-muted-foreground">
              MonClub TV {curVersion}{curCodename ? ` "${curCodename}"` : ""} is the latest version.
            </p>
            {status.lastCheckAt && (
