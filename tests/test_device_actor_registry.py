...
def test_update_devices_refreshes_existing_actor_adapter_when_supported():
    class UpdatableAdapter:
    ...
    registry = DeviceActorRegistry(
        adapter_factory=lambda device: UpdatableAdapter(device),
        actor_factory=actor_factory,
    )

    registry.update_devices([{"id": 7, "name": "A"}])
    registry.update_devices([{"id": 7, "name": "A2"}])

    assert actors[7].adapter.updated_to == {"id": 7, "name": "A2"}
